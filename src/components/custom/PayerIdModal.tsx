...
const PayerIdModal: React.FC<Props> = ({ isOpen, onClose }) => {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-lg p-6">
        <h3 className="text-lg font-semibold mb-4">PAYER ID Payment</h3>
        <p className="text-sm text-slate-600 mb-4">
          Enter PAYER ID payment details here.
        </p>

        <TextBox label="PAYER ID" placeholder="Enter your PAYER ID" />
        <div className="flex flex-col gap-2 mt-4 sm:flex-row-reverse">
          ...
          <button
            onClick={() => alert("Proceed to payment (PAYER ID).")}
